...
/* Falling snowflakes - 6 independent layers with prime-number timing */
    ...
    `}_renderBackgroundEffect(t){const e=this._getEffectClass(t);if(!e)return j``;if("effect-snow"===e){const t=this._getSnowIntensity();return j`
      ...
      `:j`<div class="weather-effects ${e}"></div>`}_getEffectClass(t){const e=t?.toLowerCase()||"";return e.includes("rain")||e.includes("pouring")?"effect-rain":e.includes("snow")?"effect-snow":e.includes("fog")?"effect-fog":"sunny"===e||"clear"===e?"effect-sunny":""}_getSnowIntensity(){const t=this._config.weather_entity?this.hass.states[this._config.weather_entity]:void 0;if(!t)return"";const e=Number(t.attributes?.precipitation)||0;return e>=4?"intensity-heavy":e<1?"intensity-light":""}_renderWeatherIcon(t,e){if(!t)return j`<span class="unavailable">No weather entity</span>`;const i=function(t,e){const i=t?.toLowerCase().replace(/-/g,"")||"cloudy",a=e?"day":"night",r=`${i}_${a}`;if(_t[r])return _t[r];const s=`${t}_${a}`;if(_t[s])return _t[s];const n={exceptional:"cloudy"}[i];if(n){const t=`${n}_${a}`;if(_t[t])return _t[t]}return'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><text x="32" y="40" text-anchor="middle" font-size="24">?</text></svg>'}(t,e);return j`${vt(i)}`}_renderSunTimes(){const t=this._config.sun_entity?this.hass.states[this._config.sun_entity]:void 0;if(!t)return j``;const e=t.attributes.next_rising,i=t.attributes.next_setting,a=t=>{if(!t)return"--:--";return new Date(t).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})};return j`
      <div class="sun-times">
        <div class="sun-time">
